...
final class ContributionIatsTest extends WebformCivicrmTestBase {
  ...
  protected function setUp(): void {
    ...
    $content = str_replace('current($processresult->AUTHORIZATIONRESULT)', '((array) $processresult->AUTHORIZATIONRESULT)[0] ?? ""', file_get_contents($f));
    $content = str_replace('current($processresult->TRANSACTIONID)', '((array) $processresult->TRANSACTIONID)[0] ?? ""', $content);
